...
def s2b(s):
  """Converts an ASCII string into binary data
...
def encode(e,spacing=''):
  """Encodes binary data int an emoji string

  Args:
      e: Data to be encoded
      spacing: A character to be inserted between each emoji. Default is none.

  Returns: 
      A string of emoji characters separated by the specified character.
  """
...
def decode(d,spacing=''):
  ...
  for v in list(gc.grapheme_clusters(d)):
    ...
    if v == spacing:
      continue
    #this is a fix for grapheme_clusters finding false groups
    #start a decode list
    decode_list = []
    #if the found cluster is 4 or more code points
    if len(v) >= 4:
      i=0
      #for every 2 code points, treat as one emoji
      while i < len(v):
        decode_list.append(v[i:i+2])
        i+=2
    else:
      #this is a normal cluster
      decode_list.append(v)
    #for each emoji, decode
    for l in decode_list:
      #start an emoji group
      e = []
      #for each code point, reverse to an ord() in hex 
      for i in range(len(l)):
        e.append(format(ord(l[i]),'x').zfill(4).upper())
      #combine the code points with hyphens to get the mapping index
      p = '-'.join(e)
      #shift the data left by 10 bits and add the index of the emoji
      r = (r<<10) + EMOJI.index(p)
  #return the data
  return r

def encodeStr(st,spacing=''):
  """Converts a string to binary data and then encodes it. Wrapper for encode().
...
def decodeStr(d,spacing=''):
  """Decodes an emoji string and then converts to a string. Wrapper for decode().
